  ...
  return (
    <div className="bg-white border-b border-gray-100 py-2.5 overflow-hidden flex items-center shadow-sm">
      ...
      <div className="flex-shrink-0 flex items-center gap-3 px-5 border-r border-gray-100 h-full">
        <span className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-violet-600 whitespace-nowrap">
          As Covered By
        </span>
      </div>
      {/* Scrolling names */}
      <div className="flex-1 marquee-wrapper">
        <div className="animate-marquee-left-fast flex items-center gap-0">
          {mediaDouble.map((name, i) => (
            <span
              key={`${name}-${i}`}
              className="inline-flex items-center gap-3 px-6 text-[11px] font-bold tracking-widest uppercase whitespace-nowrap text-gray-400 hover:text-gray-700 transition-colors"
            >
